...
def main():
    try:
        ...
        log.info(f"Is {CONTAINER_NAME} in running state? : {running_state}\n")
        ...
        while not running_state:
            ...
            cont = container.get_container(CONTAINER_NAME)
            log.info(
                f"Container restarted {counter} time(s) and its {cont.attrs['State']['Status']} after 90s..."
            )
